...
def rebuild_and_mark_error(toks: list, pointer_to_error: int) -> None:
    result = f"{bcolors.OKCYAN}\""

    for tok in toks:
        if toks[pointer_to_error] == tok:
            result += f"{bcolors.FAIL}{bcolors.UNDERLINE}{tok.value}{bcolors.ENDC}"
        else:
            result += f"{bcolors.OKCYAN}{tok.value}"

    result += f"{bcolors.OKCYAN}\"{bcolors.ENDC}"
    return result
...
def tokenize(input_str: str) -> list:
    ...
    for char in input_str:
        ...
        if char == "+":
            ...
            colorized_string = f"{bcolors.OKCYAN}\"{input_str[:input_str.index(char)]}{bcolors.FAIL}{bcolors.UNDERLINE}{input_str[input_str.index(char)]}{bcolors.ENDC}{bcolors.OKCYAN}{input_str[input_str.index(char)+1:]}\"{bcolors.ENDC}"
